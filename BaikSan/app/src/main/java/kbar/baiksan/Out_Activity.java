...
/*
<주요기능>
- 출고처리 : 스캔 - 출고중복체크 - 재고조회(출고할 재고가 있느지) - 해당바코드의 오더정보 - 정상출고처리(구분=0)
*/
public class Out_Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn_scan.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                //카메라 스캔
                IntentIntegrator integrator = new IntentIntegrator(Out_Activity.this);
        ...
        btn_can.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Intent i = new Intent(Out_Activity.this, Out_sub_Activity.class);
                startActivity(i);
                finish();
            }
        });
                ...
                try {
                    ...
                    if (ja.length() > 0) {
                        ...
                        for (int i = 0; i < ja.length(); i++) {
                            ...
                            if(col1.equals("0")){
                                select_incheck(bar,"ok");
                            }else{
                                select_incheck(bar,"e_out"); //출고중복!!!!
                            }
                        }
                    }
    ...
    //입고내역조회
    public void select_incheck(String bar,String error_ck){
        class phpDown extends AsyncTask<String, Integer, String> {
            @Override
            protected String doInBackground(String... urls) {
                StringBuilder jsonHtml = new StringBuilder();
                try {
                    URL url = new URL(urls[0]);
                    HttpURLConnection conn = (HttpURLConnection) url.openConnection(); //HTTP 연결을 만든다
                    if (conn != null) {
                        conn.setConnectTimeout(10000);
                        if (conn.getResponseCode() == HttpURLConnection.HTTP_OK) {
                            //HttpURLConnection의 결과를 받아 buffer에 쌓는다.
                            BufferedReader br = new BufferedReader(new InputStreamReader(conn.getInputStream(), "UTF-8"));
                            for (; ; ) {
                                String line = br.readLine();
                                if (line == null) break;
                    ...
                    if (ja.length() > 0) {
                        ...
                        for (int i = 0; i < ja.length(); i++) {
                            ...
                            if(error_ck.equals("e_out")){
                                select_outqty(col1, col8, col11,"ok");
                                goDialog("확  인", "이미 출고처리된 바코드입니다.");
                            }
                            else if(error_ck.equals("e_in")){
                                select_outqty(col1, col8, col11,"ok");
                                goDialog("확  인", "입고처리되지않은 바코드입니다.");
                            }
                            else{
                                insert_out(bar,col1,col2,col9,col8,col11); //출고처리
                            }
    ...
    //출고처리
    public void insert_out(String bar,String order_no, String roll_no, String s_qty, String oqty, String ea){
        class phpUp extends AsyncTask<String, Void, String> {
            @Override
            protected String doInBackground(String... params) {

                try {
                    String data = MainActivity.App_path + "insert_out.php?";
                    data += URLEncoder.encode("bar", "UTF-8") + "=" + URLEncoder.encode(bar, "UTF-8");  //바코드
                    data += "&" + URLEncoder.encode("gubun", "UTF-8") + "=" + URLEncoder.encode("0", "UTF-8");  //0:정상출고 1:주문취소출고
        ...
        task1.execute();
        Toast.makeText(Out_Activity.this, "정상출고 완료!\n(" + bar + ")", Toast.LENGTH_SHORT).show();
        select_outqty(order_no, oqty, ea,"no");
        Escan.setText("");
        Escan.requestFocus();
    }

    //출고수량 확인
    public void select_outqty(String orderno, String oqty, String ea, String error_ck){
        class phpDown extends AsyncTask<String, Integer, String> {
            @Override
            protected String doInBackground(String... urls) {
                StringBuilder jsonHtml = new StringBuilder();
...
/*                                if(error_ck.equals("no")){
                                    goDialog("확 인","해당 오더의 요청발주수량 출고가 완료되었습니다.");
                                }*/
                            }else{
